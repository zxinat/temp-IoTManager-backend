...
namespace IoTManager.Dao.MySQL
{
    public class MySQLCustomerDao : ICustomerDao
    {
        ...
        {
            this.mysqlConnectionStr = Constant.getDatabaseConnectionString();
        }
        /* 获取所有外来人员信息
         * 分页功能
         * 部分筛选功能
        ...
        {
            string sql = "INSERT INTO customerauth(customer,device) VALUES " +
                "(@cu,(SELECT id FROM device WHERE hardwareDeviceID=@did))";
            using (var connection = new MySqlConnection(mysqlConnectionStr))
            {
